...
# Reading in the downloaded file crashes RStudio for me, so I am working with
# a local version
xl <- "data/czlma903.xls"

# Read commuting zone county partition, add place and state variables
cz <- 
  read_excel(xl, sheet = "CZLMA903", na = ".") %>% 
  select(
...
for (resolution in resolutions) {
  ...
  zones <- us %>%
    ...
    group_by(cz_1990, place) %>% # Identical, but I want to retain the name and the code
    summarize() %>% # summarize uses st_union internally to union the features within groups
